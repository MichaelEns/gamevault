...
/**
 * How long the cookies EA last issued are valid for.
 *
 * Reported rather than assumed, because it decides whether the six-hourly
 * schedule can sustain itself: each run refreshes the cookie it used, and that
 * only works while the cookie outlives the gap between runs.
 */
export let cookieLifetimes = null;
...
export async function accessToken(env, { fresh = false } = {}) {
  ...
  for (const c of setCookies) {
    ...
    // Record how long each cookie is good for. The scheduled build runs every
    // six hours, so a cookie that lives less than that breaks the chain: each
    // run refreshes the cookie it used, and the refresh only sustains itself
    // while the cookie outlives the gap between runs. Guessing at this was
    // pointless when EA states it outright.
    const maxAge = c.match(/max-age=(\d+)/i);
    const expires = c.match(/expires=([^;]+)/i);
    if (m) {
      let life = null;
      if (maxAge) life = Number(maxAge[1]) / 3600;
      else if (expires) life = (Date.parse(expires[1]) - Date.now()) / 3600000;
      if (life !== null && Number.isFinite(life)) {
        expiries.push(`${m[1].trim()}=${life.toFixed(1)}h`);
      } else {
        expiries.push(`${m[1].trim()}=session`);
      }
    }
  }
  if (expiries.length) cookieLifetimes = expiries.join(', ');
