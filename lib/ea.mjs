...
/**
 * The replacement cookie EA issued during the last exchange, if any.
 *
 * EA appears to rotate `remid` on use: a value that authenticates once then
 * fails on the next attempt. That is consistent with what happened here - a
 * run reported the account name successfully, and the following run was
 * refused outright with the same stored cookie.
 *
 * A rotating credential cannot be stored as a fixed secret without capturing
 * the replacement, so the replacement is captured. Lutris gets away with this
 * because it keeps a cookie jar and writes the new value back; the equivalent
 * here is persisting it locally and updating the GitHub secret in CI.
 */
export let rotatedRemid = null;
...
export async function accessToken(env, { fresh = false } = {}) {
  ...
  // A direct fetch rather than req(), because the Set-Cookie header matters
  ...
  // Capture a replacement before anything can throw, so a rotated cookie is
  // never lost just because the rest of the call failed.
  for (const c of res.headers.getSetCookie?.() ?? []) {
    const m = c.match(/^remid=([^;]+)/);
    if (m && m[1] && m[1] !== env.EA_REMID) rotatedRemid = m[1];
  }
  ...
  if (data?.error === 'login_required' || data?.error_code === 'login_required') {
    throw new Error(
      'EA rejected the stored session (login_required). EA rotates this cookie ' +
      'when it is used, so a value that worked once is often dead afterwards - ' +
      'copy a fresh remid from the browser and try again.',
    );
  }
