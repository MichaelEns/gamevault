...
export async function currentFreebies(env, index = {}) {
  ...
  // Each source is independent: one failing must not hide the others, since a
  // missed giveaway cannot be recovered later.
  const [epic, gog] = await Promise.all([
    epicFreeGames(country).catch(() => []),
    import('./gog-claim.mjs').then((m) => m.currentGiveaway()).catch(() => []),
  ]);
  return annotateOwnership([...epic, ...gog], index);
}
