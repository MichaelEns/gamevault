...
export async function accessToken(env) {
  ...
  if (data.refresh_token && data.refresh_token !== env.GOG_REFRESH_TOKEN) {
    ...
    // Stored immediately: see lib/credential-refresh.mjs for why the delay
    // between issue and store is a race and not a theoretical one.
    const { persistRotated } = await import('./credential-refresh.mjs');
    await persistRotated(env, 'GOG_REFRESH_TOKEN', data.refresh_token);
  }
  accessCache = {
