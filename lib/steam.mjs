 ...
 *
 * Whatever is in STEAM_ID may well be a credential -- pasting the API key
 * into the wrong field is an easy mistake, and it is exactly what happened.
 * The resulting message is stored in the snapshot and shown in the app, so it
 * must never contain the value. GitHub masks registered secrets in workflow
 * logs, but that is its safety net, not ours, and it does not apply anywhere
 * else the message travels.
 */
function describeValue(v) {
  const s = String(v ?? '');
  if (/^[0-9a-f]{32}$/i.test(s)) return 'a 32-character hex string, which looks like a Steam API key';
...
export async function resolveVanity(vanity, key) {
  ...
  if (data?.response?.success !== 1) {
    throw new Error(`Steam could not resolve that profile name (${describeValue(vanity)})`);
  }
...
export async function toSteamId64(input, key) {
  ...
  // Catch the specific mix-up before spending a request on it, and before the
  // value can end up in an error message.
  if (!vanityUrl && /^[0-9a-f]{32}$/i.test(vanity)) {
    throw new Error(
      'STEAM_ID looks like a Steam API key (32 hex characters), not a profile. ' +
      'It should be your profile URL, e.g. https://steamcommunity.com/id/yourname, ' +
      'or a 17-digit SteamID64. The API key belongs in STEAM_API_KEY.',
    );
  }

  if (/^\d+$/.test(vanity)) {
    throw new Error(
      `STEAM_ID is ${vanity.length} digits; a SteamID64 is 17. That is probably ` +
