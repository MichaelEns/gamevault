 ...
 *
 * Shared by configured() and readLegendaryConfig() on purpose. When these were
 * two lists they disagreed: configured() looked only at environment variables,
 * so on a developer machine with a perfectly good legendary login the tools
 * refused to start with "Epic is not set up yet", while the code that would
 * have done the work could read the credential from disk quite happily.
 */
...
export function configured(env) {
  if (env.EPIC_COOKIES || env.LEGENDARY_CONFIG || env.GAMEVAULT_LEGENDARY_BIN) {
    return true;
  }
  if (env.LEGENDARY_USER_JSON) return true;
...
async function launcherToken(env) {
  ...
  if (data.refresh_token && data.refresh_token !== stored.refresh_token) {
    ...
    // Outside CI there is no secret to write, so the replacement goes back to
    // the file it came from. Without this the rotation is simply lost.
    if (!saved) {
      ...
      if (wroteBack) {
        // persistRotated has just warned that the next build will fail, which
        // is true of the secret but not of this machine. Say so, or the run
...
async function readLegendaryConfig(env) {
  ...
  try {
    ...
    for (const p of legendaryUserPaths()) {
      try {
        const parsed = JSON.parse(await readFile(p, 'utf8'));
        legendarySourcePath = p;
        return parsed;
      } catch { /* next */ }
    }
  } catch { /* fall through */ }
...
/**
 * Write a rotated token back into legendary's own config.
 *
 * Epic retires a refresh token the moment it is used, so a rotation that is
 * received and not stored destroys the login. In CI that is handled by
 * persistRotated writing the GitHub secret, but a developer running these
 * tools by hand has no secrets token, so the replacement had nowhere to go:
 * running the local verification tool would silently burn a credential that
 * was otherwise good for a year, and the next run would report the login as
 * expired. The tool meant to check the setup was the thing breaking it.
 *
 * Best-effort and never fatal - failing to write is worth a warning, not a
 * lost claim. Written via a temporary file and renamed so an interrupted
 * write cannot leave legendary with a truncated config.
 */
...
/**
 * What to tell the operator when Epic rejects an otherwise well-formed request.
 *
 * The credential to renew depends on which branch of sessionCookies() supplied
 * the session, and naming the wrong one sends people to fix a secret that does
 * not exist. EPIC_COOKIES is the rarely-used override; the normal path derives
 * the session from legendary, so the thing to renew is LEGENDARY_CONFIG.
 *
 * On that normal path the message must not claim the stored login "expired".
 * By the time these 401s can fire, the refresh token has already been
 * exchanged for an access token and that access token exchanged for a web
 * session - both successfully, seconds earlier. Whatever Epic is objecting to,
 * it demonstrably is not a lapsed refresh token, and saying otherwise sends
 * someone to re-run a setup step that changes nothing.
 */
function sessionExpiredError(env, where) {
  if (env.EPIC_COOKIES) {
    return new Error(
      `Epic rejected the session${where} (401). The EPIC_COOKIES override has ` +
      'expired - replace it, or unset it to use the legendary login instead.');
  }
  return new Error(
    `Epic rejected the session${where} (401). The LEGENDARY_CONFIG login was ` +
    'exchanged successfully just before this, so it has not simply expired - ' +
    'Epic declined the web session derived from it. Re-running ' +
    '".\\finish-setup.ps1 -Only epic" is the first thing to try; if it happens ' +
    'again the session exchange itself is what needs looking at.');
}
...
export async function claim(env, game) {
  if (!configured(env)) {
    throw new Error(
      'Epic is not set up: no legendary login was found and none of ' +
      'LEGENDARY_CONFIG, EPIC_COOKIES or GAMEVAULT_LEGENDARY_BIN is set. ' +
      'Run ".\\finish-setup.ps1 -Only epic".',
    );
  }
  ...
  if (res.status === 401) {
    throw sessionExpiredError(env, ' while ordering');
  }
  if (data?.errorCode) {
