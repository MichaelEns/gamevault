...
function sessionExpiredError(env, where) {
  const how = env.EPIC_COOKIES
    ? 'The EPIC_COOKIES override has expired - replace it, or unset it to use ' +
      'the legendary login instead.'
    : 'The Epic login stored in LEGENDARY_CONFIG has expired - re-run ' +
      '".\\finish-setup.ps1 -Only epic", which signs in again and updates the ' +
      'secret for you.';
  return new Error(`Epic rejected the session${where} (401). ${how}`);
}
