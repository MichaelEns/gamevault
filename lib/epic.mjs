...
/**
 * Run legendary and preserve any refresh token it wrote, even on failure.
 *
 * Authentication happens before catalogue fetching. A later timeout can make
 * the command fail after the old refresh token has already been replaced, so
 * persistence belongs in finally rather than after a successful command.
 */
export async function runLegendary(args, options, {
  bin = legendaryPath(),
  env = process.env,
  exec = execFileAsync,
} = {}) {
  try {
    return await exec(bin, args, options);
  } finally {
    try {
      const { persistLegendaryRefreshToken } = await import('./epic-claim.mjs');
      await persistLegendaryRefreshToken(env);
    } catch (e) {
      console.log(`::warning::Could not preserve legendary's refresh token: ${e.message}`);
    }
  }
}
...
export async function authStatus() {
  ...
  try {
    const { stdout } = await runLegendary(['status', '--json'], {
      timeout: 30000,
      windowsHide: true,
    }, { bin });
...
export async function ownedGames() {
  ...
  const { stdout } = await runLegendary(['list', '--json'], {
    timeout: 120000,
