...
const withAlertId = (entry) => entry.alertId
  ? entry
  : { ...entry, alertId: randomBytes(12).toString('hex') };
...
export function verifyClaims(log, index = {}, now = Date.now()) {
  ...
  for (const entry of log) {
    ...
    if (entry.manual) {
      ...
      // Once the giveaway ends there is no useful action left to report. Drop
      // the tombstone too, so a later repeat giveaway can be surfaced afresh.
      if (Number.isFinite(endsAt) && endsAt <= now) continue;
      failed.push({
        ...entry,
        failedAt: entry.failedAt ?? entry.attemptedAt ?? new Date(now).toISOString(),
        reason: entry.error ?? 'browser confirmation is required',
      });
      continue;
    }
...
export function nextLog({ pending, failed, attempts }) {
  const out = new Map();
  for (const p of pending) out.set(entryKey(p), withAlertId(p));
  ...
  for (const f of failed) {
    ...
    out.set(entryKey(f), withAlertId(tries >= MAX_ATTEMPTS
  ...
  for (const a of attempts) {
    ...
    const fresh = withAlertId(a);
    out.set(entryKey(a), prior
      ? { ...fresh, alertId: prior.alertId, attempts: (prior.attempts ?? 1) + 1 }
      : fresh);
  }
...
export function recordAttempt(game, {
  ok,
  error = null,
  manual = false,
  giveUp = false,
  ...
  const entry = {
    ...
    error,
    attempts: 1,
    alertId: randomBytes(12).toString('hex'),
  };
  if (manual) entry.manual = true;
  if (giveUp) entry.giveUp = true;
  return entry;
}
...
export function reportable(log, now = Date.now()) {
  return log.filter((e) => {
    ...
    if (e.manual) {
      ...
      // A long-running promotion must not look "cleared" merely because the
      // generic failure window elapsed while the browser action is still live.
...
/**
 * What kind of failure this is, as one of five fixed words.
 *
 ...
 *   manual   - this store requires browser confirmation for the free checkout.
 *   error    - anything else.
 *
 ...
 *
 * Each id is random and stored only inside the encrypted claim log. Deriving it
 * from the snapshot passphrase would turn a known public giveaway title into a
 * cheap offline passphrase oracle, bypassing the snapshot's expensive PBKDF2.
 */
export function publicAlerts(log, { now = Date.now() } = {}) {
  return reportable(log, now).map((e) => {
    if (!/^[0-9a-f]{24}$/.test(e.alertId ?? '')) {
      throw new Error('Claim entry is missing its opaque alert id');
    }
    return {
      id: e.alertId,
