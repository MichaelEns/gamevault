...
/**
 * Read an Actions variable. Unlike secrets, variables ARE readable, which is
 * what makes appending to the watchlist possible from the app rather than
 * silently overwriting whatever was already there.
 *
 * Returns '' when the variable does not exist yet, so callers can treat a
 * first-ever write and an append identically.
 */
export async function getVariable(token, repo, name) {
  const res = await api(token, `/repos/${repo}/actions/variables/${encodeURIComponent(name)}`);
