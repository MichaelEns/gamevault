...
export async function req(url, opts = {}) {
  ...
  // The underlying cause is on e.cause, so unwrap it and say which host could
  // not be reached - the difference between "my key is wrong" and "this host
  // refuses connections from CI" is the whole diagnosis.
  if (lastErr && /fetch failed/i.test(lastErr.message ?? '')) {
    let host = url;
    try { host = new URL(url).host; } catch { /* keep the raw string */ }
    const cause = lastErr.cause?.code ?? lastErr.cause?.message ?? 'no further detail';
    const err = new Error(
      `Could not reach ${host} (${cause}). ` +
      'This is a network failure rather than a rejected credential; some ' +
      'services refuse connections from datacenter IP ranges, which includes ' +
