...
async function login(env, { force = false } = {}) {
  ...
  try {
    ...
    if (e.status === 401) {
      ...
      throw new Error(env.UBISOFT_REMEMBER_TICKET
        ? 'Ubisoft rejected the remember-me ticket (401). Ubisoft retires the ' +
          'ticket the moment it is used, so this means the stored one had ' +
          'already been spent - either by the previous build, or by another ' +
          'build that overlapped this one. Re-run "npm run ubisoft-auth"; if it ' +
          'keeps happening, check GAMEVAULT_SECRETS_TOKEN can write secrets, ' +
          'because that is what saves each replacement.'
  ...
  if (data.rememberMeTicket && data.rememberMeTicket !== env.UBISOFT_REMEMBER_TICKET) {
    ...
    // Stored NOW rather than at the end of the build. Builds queue but still
    // overlap by seconds, and a build that starts before the write lands reads
    // the ticket this login just spent. Persisting here shrinks that window
    // from minutes to one API call.
    const { persistRotated } = await import('./credential-refresh.mjs');
    await persistRotated(env, 'UBISOFT_REMEMBER_TICKET', data.rememberMeTicket);
  }
